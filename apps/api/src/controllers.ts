...
export class AppController {
  ...
  @Roles(Role.OWNER)
  staff(@Req() req: any) {
    return this.prisma.user.findMany({ where: { businessId: req.user.businessId } });
  }
  ...
  @Get('customers')
  @UseGuards(AuthGuard)
  customers(@Req() req: any, @Query('q') q?: string, @Query('page') page = '1') {
  ...
  async createJob(@Req() req: any, @Body() body: CreateJobDto) {
    const j = await this.prisma.job.create({ data: { ...body, businessId: req.user.businessId } });
  ...
  job(@Req() req: any, @Param('id') id: string) {
    return this.prisma.job.findFirst({
      ...
      include: { notes: true, photos: true, customer: true, quotes: { include: { items: true } }, invoices: { include: { items: true } } },
    });
  }
  ...
  async patchJob(@Req() req: any, @Param('id') id: string, @Body() body: UpdateJobDto) {
    const j = await this.prisma.job.update({ where: { id, businessId: req.user.businessId }, data: body });
  ...
  @UseGuards(AuthGuard)
  quotes(@Req() req: any) {
    return this.prisma.quote.findMany({ where: { businessId: req.user.businessId }, include: { items: true }, orderBy: { createdAt: 'desc' } });
  }
  ...
  @Post('quotes/:id/send')
  @UseGuards(AuthGuard)
  qsend(@Req() req: any, @Param('id') id: string) {
    return this.prisma.quote.update({ where: { id, businessId: req.user.businessId }, data: { status: QuoteStatus.SENT } });
  }

  @Post('quotes/:id/accept')
  @UseGuards(AuthGuard)
  qaccept(@Req() req: any, @Param('id') id: string) {
    return this.prisma.quote.update({ where: { id, businessId: req.user.businessId }, data: { status: QuoteStatus.ACCEPTED } });
  }

  @Post('quotes/:id/decline')
  @UseGuards(AuthGuard)
  qdecline(@Req() req: any, @Param('id') id: string) {
    return this.prisma.quote.update({ where: { id, businessId: req.user.businessId }, data: { status: QuoteStatus.DECLINED } });
  }
  ...
  async invoices(@Req() req: any) {
    ...
    return list.map((i) => this.svc.overdue(i));
  }
  ...
  async fromQuote(@Req() req: any, @Param('quoteId') quoteId: string) {
    const q = await this.prisma.quote.findFirstOrThrow({
      where: { id: quoteId, businessId: req.user.businessId },
      include: { items: true },
    });
    return this.createInvoice(req, {
      jobId: q.jobId,
      discountAmount: Number(q.discountAmount),
      items: q.items.map((i) => ({ description: i.description, quantity: Number(i.quantity), unitPrice: Number(i.unitPrice) })),
    } as CreateInvoiceDto);
  }
  ...
  async invoice(@Req() req: any, @Param('id') id: string) {
    ...
    return i ? this.svc.overdue(i) : i;
  }

  @Patch('invoices/:id')
  @UseGuards(AuthGuard)
  async patchInvoice(@Req() req: any, @Param('id') id: string, @Body() body: UpdateInvoiceDto) {
    const i = await this.prisma.invoice.update({ where: { id, businessId: req.user.businessId }, data: body });
    await this.svc.audit(req.user, 'Invoice', id, 'update', body);
    return i;
  }
  ...
  sendInvoice(@Req() req: any, @Param('id') id: string) {
    return this.prisma.invoice.update({ where: { id, businessId: req.user.businessId }, data: { status: InvoiceStatus.SENT } });
  }
  ...
  async markPaid(@Req() req: any, @Param('id') id: string, @Body() body: MarkPaidDto) {
    const payment = await this.prisma.payment.create({
      data: {
        ...
        paidAt: new Date(body.paidAt || new Date()),
      },
    });
  ...
  async summary(@Req() req: any) {
    ...
    const jobsToday = await this.prisma.job.count({
      where: { businessId: req.user.businessId, createdAt: { gte: todayStart } },
    });
    const inv = await this.prisma.invoice.findMany({ where: { businessId: req.user.businessId } });
    ...
    return { jobsTodayCount: jobsToday, overdueInvoicesCount: overdue.length, outstandingTotal: outstanding };
  }
  ...
  async bas(@Req() req: any, @Query('from') from: string, @Query('to') to: string, @Res() res: Response) {
    const invoices = await this.prisma.invoice.findMany({
      where: { businessId: req.user.businessId, createdAt: { gte: new Date(from), lte: new Date(to) } },
    ...
    const csv = stringify(
      invoices.map((i) => ({
        ...
        customerName: i.job.customer.name,
        status: i.status,
      })),
      { header: true },
    );
  ...
  @Patch('business')
  @UseGuards(AuthGuard)
  updateBusiness(@Req() req: any, @Body() body: UpdateBusinessDto) {
    return this.prisma.business.update({ where: { id: req.user.businessId }, data: body });
  }

  @Get('business')
  @UseGuards(AuthGuard)
  getBusiness(@Req() req: any) {
    return this.prisma.business.findUnique({ where: { id: req.user.businessId } });
  }
}
